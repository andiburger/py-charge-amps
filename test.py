...
class TestCfgFileGenerator(unittest.TestCase):
    ...
    def test_prompt_cfg_interactive(self, mock_file, mock_get_key,
                                    mock_encrypt, mock_getpass, mock_input):
        # Simulate user input
        ...
        self.assertEqual(config["USERDATA"]["email"],
                         "ENCRYPTED(test@example.com)")
        self.assertEqual(config["USERDATA"]["password"],
                         "ENCRYPTED(supersecret)")
        self.assertEqual(config["USERDATA"]["apiKey"], "apikey123")
        self.assertEqual(config["GENERAL"]["baseUrl"],
                         "https://api.chargeamps.com")
...
class TestCfgLoader(unittest.TestCase):
    ...
    def testCfg2(self):
        ...
        self.assertEqual(
            userData["email"],
            "gAAAAABoB9PepOgdxVn8UklN3Z-9ofOp71qVF0du3r651xRlSY2n9c5WyK6wBPC6xEx6hXNSuy4enBChHYrW0I6fnk6Fk5T99_XnyKf7luERp1UhNDD3jXo="
        )
...
class TestPyChargeAmpsAPI(unittest.IsolatedAsyncioTestCase):
    ...
    async def testAPI(self):
        ...
        userData = cfgParser.get_user_data()
        general_data = cfgParser.get_general_data()
        myclient = Client(decrypt(userData["email"], self.key),
                          decrypt(userData["password"], self.key),
                          userData["apiKey"], general_data["baseUrl"])
        ...
        with self.subTest(msg="Test charging sessions retrieval"):
            charging_sessions = await myclient.get_connector_chargingsessions(
                charge_point_id=chargePoints[0].id, connector_id=1)
            self.assertGreater(charging_sessions[0].total_consumption_kwh, 0)
        with self.subTest(
                msg="Test charging session retrieval for specific rfid"):
            charging_sessions = await myclient.get_rfid_chargingsessions(
                charge_point_id=chargePoints[0].id,
                connector_id=1,
                rfid="9C8BE8DF")
            self.assertGreater(len(charging_sessions), 0)
            result_writer = XlsxResult()
            result_writer.gen_output_file(charging_sessions,
                                          general_data["pricekWh"])
            user = await myclient.get_user(myclient._user._userid)
            print(user)
            self.assertEqual(user.email, userData["email"])
